(() => {
  ...
  function bindTableEditors(){
    document.querySelectorAll('.dd-tbl-cell').forEach(i=>i.oninput=()=>{
      ...
      //    🔴 지우기는 **다시 그리지 않고** DOM 에서 직접 뗀다 -- 글 치는 중에 섹션을 다시
      //    그리면 커서가 맨 뒤로 튄다(기존 테스트
      //    `test_web_can_add_a_table_section_and_edit_the_table` 이 이 핸들러에 재렌더 호출이
      //    없다는 걸 **문자열로** 잠근다. 그 함수 이름을 주석에도 쓰면 테스트가 깨진다).
      delete b._pasteNote;
