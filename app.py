...
@app.route('/api/condition-survey/export')
@login_required
def api_cs_export():
    """단일 분기 survey → Defect/Observation List 엑셀. 버튼 클릭시 다운로드(+프론트가 메일드래프트).
    params: survey_id  또는  vessel_id+year+quarter."""
    from io import BytesIO
    try:
        from openpyxl import Workbook
        from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    ...
    sid = request.args.get('survey_id')
    if sid:
        s = query('SELECT * FROM cs_surveys WHERE id=?', (sid,), one=True)
    else:
        vid, yr, q = request.args.get('vessel_id'), request.args.get('year'), request.args.get('quarter')
        if not (vid and yr and q):
            return jsonify({'error': 'survey_id 또는 vessel_id+year+quarter 필요'}), 400
        s = query('SELECT * FROM cs_surveys WHERE vessel_id=? AND year=? AND quarter=?', (vid, yr, q), one=True)
    if not s:
        return jsonify({'error': 'survey 없음'}), 404
    # 권한 스코프: 비admin 감독은 본인 담당선박만 (대시보드 스코핑과 동일 원칙)
    if session.get('supervisor_id') and session.get('role') != 'admin':
        ok = query('SELECT 1 FROM supervisor_vessels WHERE supervisor_id=? AND vessel_id=?',
                   (session.get('supervisor_id'), s['vessel_id']), one=True)
        if not ok:
            return jsonify({'error': '권한 없음(담당 선박 아님)'}), 403
    s = _cs_survey_with_counts(s)
    vrow = query('SELECT name FROM vessels WHERE id=?', (s['vessel_id'],), one=True)
    vessel = (vrow['name'] if vrow else '') or ''
    findings = query('SELECT category, no, item, description, remark, status '
                     'FROM cs_findings WHERE survey_id=? ORDER BY category DESC, no', (s['id'],))

    wb = Workbook(); ws = wb.active; ws.title = f"{s['quarter']}Q Survey"
    hdr_fill = PatternFill('solid', fgColor='1F4E78'); hdr_font = Font(bold=True, color='FFFFFF', size=10)
    thin = Side(style='thin', color='BFBFBF'); border = Border(left=thin, right=thin, top=thin, bottom=thin)
    wrap = Alignment(wrap_text=True, vertical='top')
    # 상단 정보
    title = (f"M/V {vessel} — {s.get('vendor') or ''} Survey Result & Defect List "
             f"({s['quarter']}Q {s['year']})")
    ws['A1'] = title; ws['A1'].font = Font(bold=True, size=13); ws.merge_cells('A1:H1')
    ws['A2'] = (f"Inspection Date: {s.get('inspection_date') or '-'}    "
                f"Defects: {s['defect_count']}    Observations: {s['observation_count']}    "
                f"(Total {s['total_count']} / Closed {s['close_count']})")
    ws['A2'].font = Font(size=10, color='555555'); ws.merge_cells('A2:H2')
    cols = ['No', 'Category', 'Item', 'Finding / Description', 'Remark',
            'Corrective Action Taken', 'Photo', 'OPEN/CLOSE']
    widths = [6, 12, 22, 44, 26, 34, 12, 12]
    hr = 4
    for i, (c, w) in enumerate(zip(cols, widths), start=1):
        cell = ws.cell(row=hr, column=i, value=c)
        cell.fill = hdr_fill; cell.font = hdr_font; cell.alignment = Alignment(horizontal='center', vertical='center')
        cell.border = border; ws.column_dimensions[get_column_letter(i)].width = w
    r = hr + 1
    for f in findings:
        vals = [f['no'], f['category'], f['item'], f['description'], f['remark'],
                '', '', ('CLOSE' if str(f['status'] or '').strip().lower() == 'closed' else 'OPEN')]
    ...
    bio = BytesIO(); wb.save(bio); bio.seek(0)
    def _safe(x): return re.sub(r'[^\w.\-]', '_', str(x or ''))
    fname = f"{_safe(vessel)}_{_safe(s.get('vendor'))}_{s['quarter']}Q{s['year']}_DefectList.xlsx"
