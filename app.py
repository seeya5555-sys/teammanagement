...
def init_db(drop=False):
    ...
    try:
        ...
        # ── Daily Handoff(일일 핸드오프 노트) ──
        #   맥(handoff_gen.py)이 그날 메일을 덴키로 3섹션 정리 → /api/ext/handoff/upsert 로 적재.
        #   섹션: today(오늘 처리)/tomorrow(내일 할 일)/pending(대기 중). 날짜당 1 노트.
        #   ⚠️ 참고 요약 전용 — 결재/금전 경로 아님. 사람이 항목 done 체크/메모/직접추가 가능.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS handoff_note (
                ddate        TEXT PRIMARY KEY,                -- YYYY-MM-DD(로컬)
                generated_at TEXT,
                source       TEXT,                            -- denki / raw
                mail_count   INTEGER NOT NULL DEFAULT 0,
                warns        TEXT,                            -- JSON 배열(폴더 누락 경고 등)
                created_at   TEXT NOT NULL DEFAULT (datetime('now','localtime')),
                updated_at   TEXT NOT NULL DEFAULT (datetime('now','localtime'))
            )
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS handoff_item (
                id         INTEGER PRIMARY KEY AUTOINCREMENT,
                ddate      TEXT NOT NULL,                     -- handoff_note.ddate
                section    TEXT NOT NULL,                     -- today / tomorrow / pending
                text       TEXT NOT NULL,
                src        TEXT NOT NULL DEFAULT 'mail',      -- mail / denki / user(사람 직접추가)
                done       INTEGER NOT NULL DEFAULT 0,        -- 사람 체크
                memo       TEXT,                              -- 사람 메모
                sort_no    INTEGER NOT NULL DEFAULT 0,
                created_at TEXT NOT NULL DEFAULT (datetime('now','localtime'))
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_handoff_item_date ON handoff_item(ddate, section)")
...
@app.route('/api/ext/handoff/upsert', methods=['POST'])
@api_key_required
def api_ext_handoff_upsert():
    """맥(handoff_gen.py)이 그날 핸드오프 노트(3섹션)를 적재. 날짜당 1 노트(upsert).
    사람이 손댄 상태 보존: 기존 auto 아이템의 done/memo를 (section,text) 매칭으로 재적용,
    사람 직접추가(src='user') 아이템은 절대 삭제 안 함."""
    d = request.get_json(silent=True) or {}
    ddate = (d.get('date') or '').strip()
    if not re.match(r'^\d{4}-\d{2}-\d{2}$', ddate):
        return jsonify({'error': 'bad date (YYYY-MM-DD)'}), 400
    try:                                                 # 실제 달력 유효성(2026-99-99 차단)
        datetime.strptime(ddate, '%Y-%m-%d')
    except ValueError:
        return jsonify({'error': 'invalid calendar date'}), 400
    sections = d.get('sections') or {}
    # 사람이 손댄 항목 = 보존(삭제 안 함): src='user'(직접추가/carry-over 승격) OR done=1 OR memo 있음.
    # 안 건드린 auto만 교체 → done/memo/carry 상태 유실 방지(올마이트 지적). dedup 위해 보존 (section,text) 수집.
    kept = set()
    for r in query("SELECT section,text FROM handoff_item WHERE ddate=? AND "
                   "(src='user' OR done=1 OR (memo IS NOT NULL AND memo!=''))", (ddate,)):
        kept.add((r['section'], r['text']))
    warns = json.dumps(d.get('warns') or [], ensure_ascii=False)
    if query("SELECT ddate FROM handoff_note WHERE ddate=?", (ddate,), one=True):
        execute("UPDATE handoff_note SET generated_at=?, source=?, mail_count=?, warns=?, "
                "updated_at=datetime('now','localtime') WHERE ddate=?",
                (d.get('generated_at'), d.get('source'), int(d.get('mail_count') or 0), warns, ddate))
    else:
        execute("INSERT INTO handoff_note(ddate,generated_at,source,mail_count,warns) VALUES(?,?,?,?,?)",
                (ddate, d.get('generated_at'), d.get('source'), int(d.get('mail_count') or 0), warns))
    # 안 건드린 auto만 삭제(touched/user 보존).
    execute("DELETE FROM handoff_item WHERE ddate=? AND src!='user' AND done=0 "
            "AND (memo IS NULL OR memo='')", (ddate,))
    n = 0
    for sec in ('today', 'tomorrow', 'pending'):
        for i, it in enumerate(sections.get(sec) or []):
            txt = (it.get('text') if isinstance(it, dict) else str(it)).strip()
            if not txt or (sec, txt) in kept:            # 보존된 항목과 중복이면 재insert 안 함
                continue
            src = (it.get('src') if isinstance(it, dict) else None) or 'mail'
            execute("INSERT INTO handoff_item(ddate,section,text,src,sort_no) VALUES(?,?,?,?,?)",
                    (ddate, sec, txt, src, i))
            n += 1
    return jsonify({'ok': True, 'date': ddate, 'items': n})


@app.route('/api/handoff/notes')
@admin_required
def api_handoff_notes():
    """노트 목록(최신순). ?from=YYYY-MM-DD&to=YYYY-MM-DD (기본 최근 21일). 각 노트에 섹션별 아이템 포함."""
    to = (request.args.get('to') or '').strip()
    frm = (request.args.get('from') or '').strip()
    where, params = [], []
    if re.match(r'^\d{4}-\d{2}-\d{2}$', frm):
        where.append("ddate >= ?"); params.append(frm)
    if re.match(r'^\d{4}-\d{2}-\d{2}$', to):
        where.append("ddate <= ?"); params.append(to)
    wsql = (" WHERE " + " AND ".join(where)) if where else ""
    notes = query(f"SELECT * FROM handoff_note{wsql} ORDER BY ddate DESC LIMIT 60", tuple(params))
    out = []
    for nt in notes:
        items = query("SELECT id,section,text,src,done,memo,sort_no FROM handoff_item "
                      "WHERE ddate=? ORDER BY section, sort_no, id", (nt['ddate'],))
        secs = {'today': [], 'tomorrow': [], 'pending': []}
        for it in items:
            secs.setdefault(it['section'], []).append(dict(it))
        try:
            warns = json.loads(nt['warns'] or '[]')
        except Exception:
            warns = []
        out.append({'date': nt['ddate'], 'generated_at': nt['generated_at'], 'source': nt['source'],
                    'mail_count': nt['mail_count'], 'warns': warns, 'sections': secs})
    return jsonify({'notes': out})


@app.route('/api/handoff/item', methods=['POST'])
@admin_required
def api_handoff_item_add():
    """사람이 직접 항목 추가(src='user'). body: {date, section, text}."""
    d = request.get_json(silent=True) or {}
    ddate = (d.get('date') or '').strip()
    section = (d.get('section') or '').strip()
    text = (d.get('text') or '').strip()
    if not re.match(r'^\d{4}-\d{2}-\d{2}$', ddate) or section not in ('today', 'tomorrow', 'pending') or not text:
        return jsonify({'error': 'date/section/text 필요'}), 400
    try:
        datetime.strptime(ddate, '%Y-%m-%d')
    except ValueError:
        return jsonify({'error': 'invalid calendar date'}), 400
    if not query("SELECT ddate FROM handoff_note WHERE ddate=?", (ddate,), one=True):
        execute("INSERT INTO handoff_note(ddate,source,mail_count) VALUES(?,'user',0)", (ddate,))
    mx = query("SELECT COALESCE(MAX(sort_no),0)+1 AS s FROM handoff_item WHERE ddate=? AND section=?",
               (ddate, section), one=True)
    execute("INSERT INTO handoff_item(ddate,section,text,src,sort_no) VALUES(?,?,?,'user',?)",
            (ddate, section, text, mx['s']))
    return jsonify({'ok': True})


@app.route('/api/handoff/item/<int:item_id>', methods=['PATCH'])
@admin_required
def api_handoff_item_patch(item_id):
    """done 토글 / memo 수정 / 섹션 이동(carry-over: tomorrow로). body: {done?, memo?, section?}."""
    d = request.get_json(silent=True) or {}
    sets, params = [], []
    if 'done' in d:
        sets.append("done=?"); params.append(1 if d.get('done') else 0)
    if 'memo' in d:
        sets.append("memo=?"); params.append((d.get('memo') or '').strip() or None)
    if 'section' in d and d.get('section') in ('today', 'tomorrow', 'pending'):
        sets.append("section=?"); params.append(d.get('section'))
        sets.append("src='user'")        # carry-over=사람 결정 → user 소유 승격(재push 시 보존)
    if not sets:
        return jsonify({'error': 'no fields'}), 400
    params.append(item_id)
    execute(f"UPDATE handoff_item SET {', '.join(sets)} WHERE id=?", tuple(params))
    return jsonify({'ok': True})


@app.route('/api/handoff/item/<int:item_id>', methods=['DELETE'])
@admin_required
def api_handoff_item_delete(item_id):
    """사람 추가 항목만 삭제 가능(auto 아이템은 보존 — 다음 upsert로 갱신)."""
    row = query("SELECT src FROM handoff_item WHERE id=?", (item_id,), one=True)
    if not row:
        return jsonify({'error': 'not found'}), 404
    if row['src'] != 'user':
        return jsonify({'error': 'auto 항목은 삭제 불가(done 체크로 처리)'}), 400
    execute("DELETE FROM handoff_item WHERE id=?", (item_id,))
    return jsonify({'ok': True})
