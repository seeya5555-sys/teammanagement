...
def main():
    ...
    # 백업은 DB 옆에 만든다(경로를 하나만 알면 되므로). 다만 운영 서버에서 `instance/` 는 매일 백업
    # tar 에 통째로 담기는 디렉토리라 그대로 두면 백업 크기가 배로 뜬다 — 확인 후 밖으로 옮길 것.
    print('  ↳ 검증 끝나면 이 백업을 앱 데이터 디렉토리 밖으로 옮길 것(백업 tar 팽창 방지)')
